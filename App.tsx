...
const App: React.FC = () => {
  ...
  const handleChoice = useCallback(async (choice: string) => {
    ...
    try {
        ...
        // Generate a summary every 5 turns (and not on the very first turn)
        if (newHistory.length > 1 && newHistory.length % 5 === 0) {
            try {
                ...
                // Don't block the game, just log the error and continue.
            }
        }
...
const handleInstallComponent = async (component: Component) => {
    ...
    try {
        ...
        // create a small story segment for the installation
...
const renderContent = () => {
    switch (gameState.stage) {
      case GameStage.START_SCREEN:
        return <StartScreen onStart={() => setGameState(prev => ({ ...prev, stage: GameStage.CREATION, customWorldPrompt: null }))} onWorldCreation={() => setGameState(prev => ({...prev, stage: GameStage.WORLD_CREATION}))} />;
      ...
      case GameStage.GAME_OVER:
        return <AdventureScreen gameState={gameState} startingScenario={startingScenario} onChoice={handleChoice} onEnterWorkshop={handleEnterWorkshop} onRestart={restartGame} />;
      case GameStage.COMBAT:
        if (!gameState.puppet || !gameState.enemy) {
            // This should not happen, but as a fallback
            return <AdventureScreen gameState={{...gameState, error: "Lỗi chiến đấu: Dữ liệu không hợp lệ."}} startingScenario={startingScenario} onChoice={handleChoice} onEnterWorkshop={handleEnterWorkshop} onRestart={restartGame} />;
        }
        return <CombatUI 
      ...
      case GameStage.WORKSHOP:
          if (!gameState.puppet) {
               return <AdventureScreen gameState={{...gameState, error: "Lỗi xưởng: Không tìm thấy con rối."}} startingScenario={startingScenario} onChoice={handleChoice} onEnterWorkshop={handleEnterWorkshop} onRestart={restartGame} />;
          }
        return <WorkshopUI 
            puppet={gameState.puppet}

...
const AdventureScreen: React.FC<AdventureScreenProps> = ({ gameState, startingScenario, onChoice, onEnterWorkshop, onRestart }) => {
                ...
                <div className="p-4 sm:p-6 flex-grow overflow-y-auto">
                    {gameState.currentSegment?.worldEvent && (
                        <div className="mb-6 p-3 bg-gray-800/50 border-l-4 border-gray-500 text-gray-300 italic animate-fade-in">
                            <p><span className="font-bold not-italic text-gray-400">Trong khi đó, ở một nơi khác...</span> {gameState.currentSegment.worldEvent}</p>
                        </div>
                    )}
                ...
                <div className="p-4 sm:p-6 pt-0 flex-shrink-0 mt-auto">
                    <div className="mt-6">
                        {gameState.isLoading && gameState.stage !== GameStage.GAME_OVER ? (
                            <div className="flex flex-col items-center justify-center"><LoadingSpinner /><p className="mt-2 text-sm text-red-400">Đang hiệu chỉnh các sợi dây điều khiển...</p></div>
                        ) : (
                            <ChoiceButtons segment={gameState.currentSegment} onChoice={onChoice} />
                        )}
                    </div>
                    <div className="mt-6 border-t border-red-500/20 pt-4 flex flex-col sm:flex-row justify-center items-center gap-4">
                        {gameState.puppet && gameState.puppet.sequence > 1 && (
                            <button
                                onClick={onEnterWorkshop}
                                disabled={gameState.puppet.mechanicalEssence < (100 * (10 - gameState.puppet.sequence))}
                                className="w-full sm:w-auto ui-button py-2 px-4"
                            >
                    ...
                    {(gameState.currentSegment?.choices.length === 0 || gameState.stage === GameStage.GAME_OVER) && !gameState.isLoading && (
                        <div className="text-center mt-8">
                            ...
                            <button onClick={onRestart} className="ui-button py-2 px-4">

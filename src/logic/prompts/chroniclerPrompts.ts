...
import type { Puppet, StorySegment, Clue, StartingScenario, ExplanationId, Quest, Companion, NPC, LoreEntry, FactionRelations } from '../../types';
...
${puppet.memoryFragments.map(f => `          - ${f.title}: ${f.text}`).join('\n') || "          Chưa có ký ức định hình nào."}`
    ...
    const defaultLore = `
**NHẮC LẠI CÁC ĐỊNH LUẬT CỐT LÕI CỦA THẾ GIỚI:**
- **Phương Pháp Đóng Vai (Persona):** Hành động phù hợp với "Nhân Cách" của con rối sẽ tăng **Cộng Hưởng (Resonance)**.
- **Mất Kiểm Soát:** Hành động mâu thuẫn sẽ giảm Cộng Hưởng và có thể tăng **Tà Năng (Aberrant Energy)**, dẫn đến **Đột Biến (Mutations)** và các ảnh hưởng tâm lý (ảo giác, hoang tưởng). Hậu quả này luôn logic với Lộ Trình của con rối.
- **Thăng Tiến:** Cần **Tinh Hoa Cơ Khí (Mechanical Essence)** và hoàn thành **Nghi Thức Thăng Tiến** để lên Thứ Tự mới.
- **Thế Giới Sống Động:** Các **NPC** sẽ ghi nhớ và phản ứng với hành động của người chơi. Trạng thái của các **địa điểm** có thể thay đổi. Luôn duy trì sự nhất quán.
    `;
...
**Ghi Nhớ:** Hãy duy trì sự nhất quán với thế giới tùy chỉnh này. Diễn giải các khái niệm game (Puppet, Resonance, Aberrant Energy, Companions, Quests) một cách sáng tạo để phù hợp với bối cảnh. Ví dụ, trong một thế giới fantasy, 'Resonance' có thể là 'Sự Hòa Hợp với Thần Linh', và 'Aberrant Energy' có thể là 'Sự Vấy Bẩn của Hắc Ám'.
    ...
    const npcContext = npcs.length > 0 ? `Các NPC Đã Gặp:\n${npcs.map(n => {
        ...
        const factionStr = n.faction ? ` (Phe: ${n.faction})` : '';
        return `- ${n.name}${factionStr} (quan hệ: ${n.relationship}) tại ${n.location}${goalStr}${knowledgeStr}`;
...
` : "Người chơi chưa có khám phá quan trọng nào được ghi lại.";
    ...
    const factionContext = Object.keys(factionRelations).length > 0 ? `
**BẢNG ĐIỂM CHÍNH TRỊ (QUAN HỆ CỦA NGƯỜI CHƠI VỚI CÁC PHE PHÁI):**
${Object.entries(factionRelations).map(([faction, score]) => `- ${faction}: ${score}`).join('\n')}
` : "Người chơi chưa có mối quan hệ đáng kể nào với các phe phái.";

    return `
        Bạn là **Người Ký Sự**, Quản Trò (Game Master) kể chuyện chính cho "Cấm Kỵ Cơ Khí".
        Hãy tiếp tục câu chuyện, tuân thủ chặt chẽ các định luật cốt lõi của thế giới VÀ bối cảnh đã được thiết lập.
        ...
        **QUẢN TRÒ BẬC THẦY - NGUYÊN TẮC VÀNG (TUÂN THỦ TUYỆT ĐỐI):**
        ...
        5.  **Tạo Ra Thế Giới Sống Động:**
            ...
            *   **Sự Kiện Thế Giới:** Thỉnh thoảng (không phải mọi lúc), hãy tạo ra một \`worldEvent\` để mô tả những gì đang xảy ra ở nơi khác, làm cho thế giới có cảm giác đang chuyển động (ví dụ: xung đột giữa các phe phái, một khám phá mới của ai đó).
        6.  **Tương Tác NPC Sâu Sắc (Bộ Não Xã Hội):**
            *   **Sử Dụng Trí Nhớ Của NPC:** Khi người chơi tương tác với một NPC đã biết, BẮT BUỘC phải để **toàn bộ hồ sơ của họ** (mối quan hệ, mục tiêu, kiến thức, **phe phái**) ảnh hưởng đến lời nói và hành động của họ. Phản ứng của họ cũng phải chịu ảnh hưởng từ mối quan hệ của người chơi với phe phái của NPC đó.
        ...
        7.  **HỆ THỐNG PHE PHÁI (QUAN TRỌNG):**
            *   **Phản Ánh Quan Hệ:** Mô tả \`scene\` và các lựa chọn \`choices\` phải phản ánh mối quan hệ hiện tại của người chơi với các phe phái. Nếu người chơi bị một phe phái ghét, thành viên của phe đó sẽ hành động thù địch.
            *   **Cập Nhật Quan Hệ:** Nếu hành động của người chơi rõ ràng có lợi hoặc có hại cho một phe phái, hãy trả về sự thay đổi điểm quan hệ trong \`updatedFactionRelations\`. Ví dụ: \`{"Giáo Hội Đồng Hồ": 10, "Cơ Giới Thần Giáo": -5}\`. Chỉ bao gồm các phe phái bị ảnh hưởng.
